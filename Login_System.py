...
def LoginScreen(screen):
    ...
    NewUsrBtn = NewUserBtn(screen)
    # create the existing user button
    ExtUsrBtn = ExtUserBtn(screen)
    return NewUsrBtn, ExtUsrBtn

def NewUserBtn(screen):
    ...
    NewUsrBtn = screen.blit(newuser_textsurface, (newuser_Text_x, newuser_Text_y))
    return NewUsrBtn

def ExtUserBtn(screen):
    ...
    ExtUsrBtn = screen.blit(extuser_textsurface, (extuser_Text_x, extuser_Text_y))
    return ExtUsrBtn

def NewUserLogin(screen):
    background_image = pygame.image.load("Images/Wall.png").convert_alpha()
    screen.blit(background_image, (0, 0))
